...
----
+
Make a new directory in your home directory with `% mkdir _/.mutt`
.. Creating password file
+
You have to create your password with
`% echo set my_pass = '_mypassword_' > _/.mutt/mypass`
+
IMPORTANT: Put a space before the command so your system will not save
your plain text password in its shell history.
+
Encrypt the file with `% gpg -r _my_email_ -e _/.mutt/mypass`, where
...
`set mailcap_path = _/.mutt/mailcap`
. Make it corolful
+

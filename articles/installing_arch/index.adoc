...
manager (`+pacman+`), and some basic coreutils (`+GNU+`). That does not
seem a lot, but this is just enough to make some great things. It
installs a barebone system which allows the user to customize their
system to smallest details and making it their own. Not like `+ubuntu+`
...
official https://wiki.archlinux.org/index.php/Installation_guide%5BArch
Wiki]. They have everything and more than you will ever need.

Open the https://www.archlinux.org/%5BArch Linux Official Website] and
download the latest image file. Later on just take a flash drive and
burn it with `+rufus+` on Windows or with `+dd+` on Linux. To save you
some trouble, here is an example `+dd+` command to burn the image
...
27GB and it is called `+/dev/mmcblk0+`. For the sake of simplicity,
let's use notation `+/dev/sda+` for the name of the drive. Okay, good.
Now, launch `+cfdisk+` by typing
...
`+/usr/share/zonenifo+` and use it instead of `+_Region_+` and
`+_City_+`
+
[source,bash]
----
...
You need to exit `+chroot+` by runing `+% exit+` and reboot with
`+% reboot+`
...
IMPORTANT: Never ever run `+% sudo pacman --force+`. The only exception
is if you have a loaded gun pointed at your head (which is very
unlikely, hopefully).
